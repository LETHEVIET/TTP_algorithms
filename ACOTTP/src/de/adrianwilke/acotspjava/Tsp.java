...
import static ttp.TTPInstance.*;

import ttp.TTPSolution;
import org.apache.commons.lang3.ArrayUtils;

import static ttp.Optimisation.Optimisation.linkernTour;

public class Tsp {

    public static ttp.TTPInstance currentTTPInstance = null;
    ...
    static int[][] compute_nn_lists()
        ...
        for (node = 0; node < n; node++) { /* compute cnd-sets for all node */
            ...
            for (i = 0; i < n; i++) { /* Copy distances from nodes to the others */
                distance_vector[i] = instance.distance[node][i];                // TTP: no problem, as this is just "copying around"
                help_vector[i] = i;
            }
    ...
    public static HashMap<String, Double> tours = new HashMap<String, Double>();
//    public static TreeSet tours = new TreeSet();
...
//             tlocal = new int[t.length];
            int[] tlocal = rotateTour(t);
            double result = 0;
            // check against hashmap
            String tourConverted = Arrays.toString(tlocal);
            if (!tours.containsKey(tourConverted)) {
                // if not contained: compute.
                TTPSolution sol = ppGreedyRT3(currentTTPInstance, Integer.MAX_VALUE/*maxruntime*/, tlocal /*int[]*/, 5, 2.5, false);
                currentTTPInstance.evaluate(sol, false);
                if (debugPrint) System.out.println(". " + tours.size());
                result = (sol.getObjective()) * -1;
                tours.put(tourConverted, result);
            } else {
                if (debugPrint) System.out.println("h");
                result = tours.get(tourConverted);
            }
            return result;
        }
        ...
        for (i = 0; i < n; i++) {
            tour_length += instance.distance[t[i]][t[i + 1]];                   // TTP: !!! needs to be replaced with TTP evaluation
        }
        return tour_length;
    }

    public static int[] rotateTour(int[] t) {
        boolean debugPrint = false;
        int[] tlocal = new int[t.length];
        if (t[0] != 0) {
            if (debugPrint) System.out.println("t[0]=" + t[0] + " tlocal[0]=" + tlocal[0]);
            //rotate tour START
            int zeroIndex = 0;
            ...
            for (int i = zeroIndex; i < t.length - 1; i++)
                tlocal[i - zeroIndex] = t[i];
            for (int i = 0; i < zeroIndex; i++)
                tlocal[t.length - zeroIndex + i - 1] = t[i];
            tlocal[tlocal.length - 1] = 0; // made explicit
            //rotate tour END
            if (debugPrint) System.out.println("zeroIndex=" + zeroIndex);
            if (debugPrint) System.out.println(Arrays.toString(t));
            if (debugPrint) System.out.println(Arrays.toString(tlocal));
...
//            if (t[0]!=0) {
//                System.out.println("t[0]="+t[0]+" tlocal[0]="+tlocal[0]);
//            }
        return tlocal;
    }
