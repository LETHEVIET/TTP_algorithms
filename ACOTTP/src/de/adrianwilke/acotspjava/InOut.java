...
//            System.out.println(">>> "+line);
            // added to consider TTP files: these contain the item definitions at the bottom, so that part can be skipped
            ...
            if (!found_coord_section) {
                ...
                // added the following to consider lines where tabs are used to separate the values
                if (city_info.length == 1)
                    city_info = line.split("\\t"); //http://stackoverflow.com/questions/1635764/string-parsing-in-java-with-delimeter-tab-t-using-split
    ...
    static void fprintf_parameters(File file) {
        ...
        printToFile(file, "AcoTsp.boosting\t\t " + AcoTsp.boosting);
    }
    ...
    static void set_default_parameters()
        ...
        max_time = 10.0;
//	optimal = 1;
        optimal = Integer.MIN_VALUE;    // to turn this package into a maximiser
        branch_fac = 1.00001;
...
//        boolean mkdir = file.mkdirs();
        ...
        Tsp.instance.distance = Tsp.compute_distances();                        // TTP: no problem, just the distance matrix gets calculated
    ...
    static void printDist()
        ...
        for (i = 0; i < Tsp.n; i++) {
            ...
            for (j = 0; j < Tsp.n - 1; j++) {
                System.out.println(" " + Tsp.instance.distance[i][j]);          // TTP: no problem, only prints the distances
            }
            System.out.println(" " + Tsp.instance.distance[i][Tsp.n - 1]);      // TTP: (same as above)
            System.out.println("\n");
        }
        System.out.println("\n");
    }
