...
public class Ants {
    ...
    static double HEURISTIC(int m, int n) {
        return (1.0 / ((double) Tsp.instance.distance[m][n] + 0.1));            // TTP: maybe no problem, since this is just a heuristic
    }
    ...
    static void place_ant_TTP(ant_struct a, int step)
        /*
         * FUNCTION: place an ant on city 0
         * INPUT: pointer to ant and the number of construction steps
         * OUTPUT: none
         * (SIDE)EFFECT: ant is put on the chosen city
         */ {
    ...
    static void choose_closest_next(ant_struct a, int phase)
        ...
        for (city = 0; city < Tsp.n; city++) {
            if (a.visited[city])
                ...
                if (Tsp.instance.distance[current_city][city] < min_distance) { // TTP: maybe no problem, since this is just about the nearest neighbour
                    next_city = city;
