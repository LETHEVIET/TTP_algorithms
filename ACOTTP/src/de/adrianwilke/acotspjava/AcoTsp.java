...
import static de.adrianwilke.acotspjava.Tsp.currentTTPInstance;
...
import static ttp.Optimisation.Optimisation.ppGreedyRT3;
...
public class AcoTsp {
     ...
     * (SIDE)EFFECTS: none
     */ {
//        boolean b3 = (Ants.best_so_far_ant.tour_length <= InOut.optimal);
//        System.out.println(b3);
        return (((InOut.n_tours >= InOut.max_tours) && (Timer.elapsed_time() >= InOut.max_time - 5)) || (Ants.best_so_far_ant.tour_length <= InOut.optimal));
    }
    ...
    static void local_search()
        ...
        for (k = 0; k < Ants.n_ants; k++) {
            ...
            if ((Timer.elapsed_time() >= InOut.max_time - 5)) break;
    ...
    static void update_statistics()
        ...
        double obOld = Ants.ant[iteration_best_ant].tour_length;
        ...
        if ((Timer.elapsed_time() >= InOut.max_time - 5)) {
            int[] tlocal = Tsp.rotateTour(Ants.ant[iteration_best_ant].tour);
            sol = ppGreedyRT3(currentTTPInstance, Integer.MAX_VALUE/*maxruntime*/, tlocal /*int[]*/, 5, 2.5, false); //put one together without subsequent hillclimbing
            sol.computationTime = (long) (Timer.elapsed_time() * 1000);
        } else if (obOld < 0.95 * bestFoundSoFar && boosting) { // attempt boosting
//            System.out.print("boosting...");
            int[] tlocal = Tsp.rotateTour(Ants.ant[iteration_best_ant].tour);
            sol = ppGreedyRT3(currentTTPInstance, Integer.MAX_VALUE/*maxruntime*/, tlocal /*int[]*/, 5, 2.5, false);

//            sol = bitFlip(currentTTPInstance, sol, 60000, 1); // bitflip
            sol = Optimisation.hillClimber(currentTTPInstance, sol.tspTour, sol.packingPlan, 2, 10000, 60000); // 1+1EA
            sol = Optimisation.insertionReverse(currentTTPInstance, sol.tspTour, sol.packingPlan, 60000, 1);
            sol = bitFlip(currentTTPInstance, sol, 60000, 1); // bitflip

            sol.computationTime = (long) (Timer.elapsed_time() * 1000);

            currentTTPInstance.evaluate(sol, false);
            double obNew = sol.getObjective() * -1;
            if (obNew < obOld) {
                Ants.ant[iteration_best_ant].tour_length = obNew;
//                System.out.println(" from "+obOld+" to "+obNew);

                // update hashmap?
                Double inserted = Tsp.tours.put(Arrays.toString(tlocal), obNew);
//                if (inserted!=null) System.out.println("tours updated: "+obNew+" from "+inserted+" "+Arrays.toString(tlocal));
            }
            ;
        } else {
...
//        System.out.println();
//        Ants.ant[iteration_best_ant].tour_length = obNew;
//        if (obNew<obOld) System.out.println("boosted from "+obOld+" to "+obNew);

        // consider the reverse direction
//        if (true) {
//            obOld = Ants.ant[iteration_best_ant].tour_length;
////            System.out.println(Arrays.toString(tlocal));
//            ArrayUtils.reverse(tlocal);
////            System.out.println(Arrays.toString(tlocal));
//            sol = ppGreedyRT3(currentTTPInstance, Integer.MAX_VALUE/*maxruntime*/, tlocal /*int[]*/, 5, 2.5, false);
//
//            sol = bitFlip(currentTTPInstance, sol, 60000, 1); // bitflip
//    //        sol = Optimisation.hillClimber(currentTTPInstance, sol.tspTour, sol.packingPlan, 2, 0x7fffffff, 60000); // 1+1EA
//            sol = Optimisation.insertionReverse(currentTTPInstance, sol.tspTour, sol.packingPlan, 60000, 1);
//            sol = bitFlip(currentTTPInstance, sol, 60000, 1); // bitflip
//
//            currentTTPInstance.evaluate(sol, false);
//            obNew = sol.getObjective()*-1;
//            System.out.println("obOld="+obOld+" obNew="+obNew);
//            if (obNew<obOld) {
//                Ants.ant[iteration_best_ant].tour_length = obNew;
//                System.out.println("boostedRev from "+obOld+" to "+obNew);
//            };
//        }
        ...
        if (Ants.ant[iteration_best_ant].tour_length < bestFoundSoFar) {
            bestFoundSoFar = Ants.ant[iteration_best_ant].tour_length;
            bestSolutionFound = sol;
        } // TTP: added
    }

    public static double bestFoundSoFar = Double.MAX_VALUE;
    public static TTPSolution bestSolutionFound = null;
...
//	}
        ...
        Tsp.currentTTPInstance = new ttp.TTPInstance(ttpFile);
        /* TTP END */
        ...
        for (InOut.n_try = 0; InOut.n_try < InOut.max_tries; InOut.n_try++) {
            ...
            while (!termination_condition()) {
                ...
                if (LocalSearch.ls_flag > 0
//                        && Utilities.ran01(Utilities.seed)<0.5                                    // TTP: deactivate local search?
                )
                    local_search();
