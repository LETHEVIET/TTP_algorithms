    ...
    *cutcount = 0;
    ...
    for (i = 0; i < cuts->cutcount; i++) {
        if (cutval[i] < testtol && !cuts->cuts[i].branch
            ...
            rval = CCtsp_tighten_lpcut_in (&lg, &old, newx, &new_, stats,
                                           &improve);
            if (rval) {
        ...
        if (test == 1) {
            ...
            while (dd) {
                ...
                if (dslack >= 1.0) {
                    ...
                    rval = CCtsp_tighten_lpcut_in (&lg, dd, newx, &new_,
                                                   stats, &improve);
                    if (rval) {
                        fprintf (stderr, "CCtsp_tighten_lpcut failed\n");
                        goto CLEANUP;
        ...
        int *marks, int *in_marker)
{
    ...
    CCtsp_lpclique *hand;
    CCtsp_lpcut_in *new_ = (CCtsp_lpcut_in *) NULL;
...
*/
    ...
    *d = (CCtsp_lpcut_in *) NULL;
    ...
    new_ = CC_SAFE_MALLOC (1, CCtsp_lpcut_in);
    CCcheck_NULL (new_, "out of memory in comb_to_domino");

    CCtsp_init_lpcut_in (new_);

    new_->cliquecount = 1;
    new_->cliques = CC_SAFE_MALLOC (1, CCtsp_lpclique);
    CCcheck_NULL (new_->cliques, "out of memory in comb_to_domino");

    rval = CCtsp_copy_lpclique (hand, &new_->cliques[0]);
    CCcheck_rval (rval, "CCtsp_copy_lpclique failed");

    new_->dominos = CC_SAFE_MALLOC (c->cliquecount - 1, CCtsp_lpdomino);
    CCcheck_NULL (new_->dominos, "out of memory in comb_to_domino");
    new_->dominocount = c->cliquecount -1;
    for (i = 0; i < new_->dominocount; i++) {
        CCtsp_init_lpdomino (&new_->dominos[i]);
    }
...
*/
    ...
    *in_marker = marker;
    if (rval) {
        if (new_) CCtsp_free_lpcut_in (new_);
    }
